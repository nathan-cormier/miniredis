...
func TestStreamAdd(t *testing.T) {
	...
	t.Run("XADD MINID", func(t *testing.T) {
		...
		minID := strconv.Itoa(int(now.Add(-time.Second).UnixMilli()))
		id, err := c.Do("XADD", "mid", "MINID", minID, "*", "one", "1")
		ok(t, err)
		id, err = c.Do("XADD", "mid", "MINID", minID, "*", "two", "2")
		ok(t, err)
		now = now.Add(time.Second)
		s.SetTime(now)
		id, err = c.Do("XADD", "mid", "MINID", minID, "*", "three", "3")
		ok(t, err)
		now = now.Add(time.Second)
		s.SetTime(now)
		// advance the minID, older entries will be dropped
		minID = strconv.Itoa(int(now.Add(-time.Second).UnixMilli()))
		id, err = c.Do("XADD", "mid", "MINID", minID, "*", "four", "4")
		ok(t, err)
		...
		s.SetTime(now)
		minID = strconv.Itoa(int(now.Add(-time.Second).UnixMilli()))
		id, err = c.Do("XADD", "mid", "MINID", "~", minID, "*", "five", "5")
		ok(t, err)
